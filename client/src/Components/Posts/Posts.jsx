  ...
  return (
    <div className="Posts">
      {posts.length > 0 ? posts.reverse().map((post, id) => {
        ...
        {post._doc.format === "image" ? <img className='imagePreview' src={post._doc.format === "image" ? `data:image/jpeg;base64,${post.postBase64}` : `data:video/mp4;base64,${post.postBase64}`} alt="imagePreviewx" /> : <video src={post._doc.format === "image" ? `data:image/jpeg;base64,${post.postBase64}` : `data:video/mp4;base64,${post.postBase64}`} alt="videoplayer" controls style={{ maxWidth: '80%' }} />}
